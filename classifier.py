...
def initialize_model(model, groups=3, users=12, features=6):
    ...
    # probability_is_1_user_in_group_log rows: users, cols: log([p(class0), p(class1)])
    model.probability_is_1_user_in_group_log = np.empty([model.users, model.groups], dtype=float)

    # Initialize last_probability_is_1_data_given_parameters_log as negative infinity
    model.probability_is_1_data_given_parameters_log = None
    model.last_probability_is_1_data_given_parameters_log = -np.inf
    return model
...
def step1(model, data):
    ''' calculate probability_is_1 of each user being in group 0...n '''
    if(model.probability_is_1_data_given_parameters_log != None):
        model.last_probability_is_1_data_given_parameters_log = model.probability_is_1_data_given_parameters_log

    for row in xrange(model.users):
        model.probability_is_1_user_in_group_log[row] = (data[row,]*model.conditional_probabilities_log).sum(axis=1) + model.group_probabilities_log
        # Normalize probability_is_1_user_in_class
        model.probability_is_1_user_in_group_log[row] = model.probability_is_1_user_in_group_log[row]-logsumexp(model.probability_is_1_user_in_group_log[row,])
    return model


def step2(model, data):
    ''' use probability_is_1_user_in_class_log and the actual data to update the conditional_probabilities_log '''
    ...
    for col in xrange(model.features):
        ...
        for row in xrange(model.users):
            if data[row, col] == 1:
                feature_counts[col] += np.exp(model.probability_is_1_user_in_group_log[row])
            z_norm += np.exp(model.probability_is_1_user_in_group_log[row])
...
def step3(model):
    ...
    for group in xrange(model.groups):
        model.group_probabilities_log[group] = logsumexp(model.probability_is_1_user_in_group_log[:,group])-np.log(model.users)
    return model


def step4(model, data):
    ''' Calculate probability_is_1 of the entire data given current parameters '''
    model.probability_is_1_user_given_data_log = np.zeros(model.users, dtype=float)
    for user in xrange(model.users):
        for feature in xrange(model.features):
            if data[user][feature] == 1:
                ...
                model.probability_is_1_user_given_data_log[user] += logsumexp(p_user_given_group_log)
    model.probability_is_1_data_given_parameters_log = np.sum(model.probability_is_1_user_given_data_log)
    return model
...
def get_model_stats(model):
    ...
    keep_going = 1
    print 'Log Likelihood: ' + str(model.probability_is_1_data_given_parameters_log)
    print 'Raw Class Probabilities: ' + str(np.exp(model.group_probabilities_log))
    if (model.probability_is_1_data_given_parameters_log < model.last_probability_is_1_data_given_parameters_log):
